...
// const PaymentPage = () => {
//   const { userid, trip_id } = useParams();
//   console.log('user id', userid, "trip id ", trip_id);
...
//           "http://127.0.0.1:8006/PaymentQRAPIView/",
//           { 
//             tripid: trip_id, 
//             invateid: userid 
//           },
...
//    const getpaymentmonyfunc =async()=>{
//         try{
//           const res=await axios.get(`http://127.0.0.1:8006/Getpaymentmony/?tripid=${trip_id}&invateid=${userid}`)
//           console.log('res  ...',res.data)
//           setAmount(res.data.amount)
//         } catch (error) {
//         console.log('does not have the date of tirp')
//       }
//     }
...
//         "http://127.0.0.1:8006/create-payment/", 
//         { amount: amount }
//       );
...
//               `http://127.0.0.1:8006/verify_payment/`, 
//               {
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const getpaymentmonyfunc = async () => {
      try {
        ...
        console.log('does not have the date of trip')
      }
    }
  ...
  // Poll for payment status when QR method is selected
  useEffect(() => {
    let intervalId;

    if (paymentMethod === 'qr' && amount && !paymentChecked) {
  ...
  }, [paymentMethod, amount, paymentChecked]);

  // Check QR payment status
  const checkQRPaymentStatus = async () => {
    try {
      // You might want to create an API endpoint to check payment status
      // For now, we'll simulate checking with the backend
      const response = await axios.post('http://127.0.0.1:8006/check-payment-status/', {
        tripid: trip_id,
        invateid: userid,
        amount: amount
      });
      ...
      if (response.data.status === 'success') {
        ...
        await recordPayment(response.data.txn_id, 'successful');
  ...
  const recordPayment = async (txnId, status) => {
    try {
      const response = await axios.post('http://127.0.0.1:8006/PaymentConfirmAPIView/', {
        tripid: trip_id,
        invateid: userid,
        user_id: userid, // or get from your auth system
        txn_id: txnId,
        amount: amount,
        status: status
      });
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
        ...
        handler: async function (response) {
          try {
            ...
            if (verifyRes.data.status === "Payment Verified") {
              ...
              await recordPayment(response.razorpay_payment_id, 'successful');
              
              alert("Payment verified successfully!");
              console.log("Payment Verified");
              navigate('/payment-success');
            } else {
              await recordPayment(response.razorpay_payment_id, 'failed');
              alert("Payment verification failed!");
            }
          } catch (error) {
            console.error("Verification Error:", error);
      ...
      razorpay.on('payment.failed', async function (response) {
        ...
        await recordPayment(response.error.metadata.payment_id || 'unknown', 'failed');
        alert(`Payment failed: ${response.error.description}`);
      });
  ...
  // Manual check payment status (for QR payments)
  const handleManualStatusCheck = async () => {
    try {
      await checkQRPaymentStatus();
      alert('Payment status checked. If payment was successful, it will be recorded.');
    } catch (error) {
            ...
            <div className="text-center">
              ...
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs text-gray-400">
                  Auto-checking payment status... or click above to check manually
                </p>
              </div>
            </div>
          ) : (
