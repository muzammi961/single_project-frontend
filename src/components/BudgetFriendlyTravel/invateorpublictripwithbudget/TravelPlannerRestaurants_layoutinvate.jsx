...
const TravelPlannerRestaurantslayoutinvate = () => {
  const { invatetripid } = useParams();
  const [tripData, setTripData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // For regular users, use Redux store data
  const calculateTripData = useSelector((state) => state.app.tripDatacalculate);
  
  // Use fetched data for invited users, otherwise use Redux store data
  const finalTripData = tripData || calculateTripData;
  
  // Get restaurants data from trip data
  const restaurantsData = finalTripData?.restaurants || [];
  ...
  const getDestinationName = () => {
    return finalTripData?.destination?.name || 
           finalTripData?.destination?.address?.split(',')[0] || 
           'Your Destination';
  };
...
export default TravelPlannerRestaurantslayoutinvate;