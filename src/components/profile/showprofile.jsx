...
export default function ProfileOnly() {
  ...
  const commentSockets = useRef(new Map());
  const likeSockets = useRef(new Map());
  ...
  // WebSocket Functions
  const initializeCommentSocket = (postId) => {
    if (commentSockets.current.has(postId)) {
      return commentSockets.current.get(postId);
    }

    const socket = new WebSocket(`ws://127.0.0.1:8004/ws/comments/${postId}/`);
    ...
    socket.onerror = (error) => {
      console.error(`💬 Comment WebSocket error for post ${postId}:`, error);
    };

    socket.onclose = () => {
      console.log(`💬 Comment WebSocket closed for post ${postId}`);
      commentSockets.current.delete(postId);
    };

    commentSockets.current.set(postId, socket);
    return socket;
  };

  const initializeLikeSocket = (postId) => {
    if (likeSockets.current.has(postId)) {
      return likeSockets.current.get(postId);
    }

    const socket = new WebSocket(`ws://127.0.0.1:8004/ws/likes/${postId}/`);
    ...
    socket.onmessage = (e) => {
      ...
      // Update the specific post with new like data
      setPosts(prevPosts => 
        prevPosts.map(post => {
          if (post.id === postId) {
            return {
    ...
    socket.onerror = (error) => {
      console.error(`❤️ Like WebSocket error for post ${postId}:`, error);
    };

    socket.onclose = () => {
      console.log(`❤️ Like WebSocket closed for post ${postId}`);
      likeSockets.current.delete(postId);
    };

    likeSockets.current.set(postId, socket);
    return socket;
  };

  const initializeNotificationSocket = () => {
    if (notificationSocket.current) {
      return notificationSocket.current;
    }

    const socket = new WebSocket(`ws://127.0.0.1:8004/ws/notifications/${authuserid}/`);
  ...
  const getNotificationMessage = (data) => {
    switch (data.notification_type) {
      case 'LIKE':
        return `User${data.sender_id} liked your post`;
      case 'COMMENT':
        return `User${data.sender_id} commented on your post`;
      case 'FOLLOW':
        return `User${data.sender_id} started following you`;
      default:
  ...
  const sendComment = (postId, text) => {
    const socket = commentSockets.current.get(postId);
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      console.error("Comment WebSocket not connected");
    }
  };

  const toggleLike = (postId) => {
    const socket = likeSockets.current.get(postId);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify({ 
        user_id: authuserid 
      }));
    } else {
      console.error("Like WebSocket not connected");
    }
  };

  // Close all WebSocket connections
  const closeAllWebSockets = () => {
    commentSockets.current.forEach((socket, postId) => {
      if (socket.readyState === WebSocket.OPEN) {
        socket.close();
      }
    });
    commentSockets.current.clear();

    likeSockets.current.forEach((socket, postId) => {
      if (socket.readyState === WebSocket.OPEN) {
        socket.close();
      }
    });
    likeSockets.current.clear();

    if (notificationSocket.current && notificationSocket.current.readyState === WebSocket.OPEN) {
  ...
  const FetchuserVideoandimages = async () => {
    try {
      ...
      if (datas.data && datas.data.data) {
        ...
        // Initialize WebSockets for each post
        experiencesData.forEach(post => {
          initializeCommentSocket(post.id);
          initializeLikeSocket(post.id);
        });
      }
    } catch (e) {
      console.log("Experience fetch failed", e);
  ...
  useEffect(() => {
    ...
    // Initialize notification WebSocket for authenticated user
    if (authuserid) {
  ...
  const handleAddComment = () => {
    ...
    sendComment(selectedPost.id, newComment);
                  ...
                  <div className="flex items-center space-x-2 mb-3">
                    <button 
                      className={`flex items-center space-x-1 text-sm ${
                        selectedPost.isLiked ? 'text-red-500' : 'text-gray-500 hover:text-teal-600'
                      }`}
                      onClick={() => toggleLike(selectedPost.id)}
                    >
                      <span className={`material-symbols-outlined ${
