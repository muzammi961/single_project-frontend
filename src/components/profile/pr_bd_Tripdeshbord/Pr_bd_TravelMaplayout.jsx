...
const Pr_bd_TravelMaplayout = () => {
  ...
  useEffect(() => {
    if (trip_id) {
      console.log('Fetching trip data for:', trip_id);
      fetchspecifictripdata();
    }
  }, [token, trip_id]);

  // Get trip data with proper fallbacks and coordinate handling
  ...
  const hasTripData = getCoordinates(destination) || getCoordinates(origin);
  ...
  const plotMapData = (map) => {
    ...
    // Plot origin with fallback coordinates
      ...
      const originMarker = L.marker([originCoords.lat, originCoords.lng], { icon: originIcon })
        ...
        .bindPopup(`
          <div class="p-3 min-w-[250px]">
            ...
            <p class="text-sm"><strong>Coordinates:</strong> ${originCoords.lat?.toFixed(6) || 'N/A'}, ${originCoords.lng?.toFixed(6) || 'N/A'}</p>
          </div>
        `);
      bounds.extend([originCoords.lat, originCoords.lng]);
      goingPoints.push([originCoords.lat, originCoords.lng]);
    }

    // Plot intermediate stops (going) - using daily itineraries for accurate route
    dailyItineraries.forEach((day, dayIndex) => {
      if (day.trip_phase === 'going' && day.next_stop) {
        const stopCoords = getCoordinates(day.next_stop);
        if (stopCoords && stopCoords.lat && stopCoords.lng) {
            ...
            html: `
              <div class="relative">
                <div class="w-5 h-5 bg-orange-500 rounded-full border-2 border-white shadow-lg flex items-center justify-center">
                  <span class="text-white text-xs font-bold">${day.phase_day}</span>
                </div>
                <div class="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                  Day ${day.phase_day}
                </div>
          ...
          const stopMarker = L.marker([stopCoords.lat, stopCoords.lng], { icon: stopIcon })
            .addTo(map)
            .bindPopup(`
              <div class="p-3 min-w-[250px]">
                <div class="flex items-center gap-2 mb-2">
                  <span class="material-symbols-outlined text-orange-500">location_on</span>
                  <strong class="text-lg">📍 Day ${day.phase_day}</strong>
                </div>
                <p class="text-sm mb-1"><strong>Address:</strong> ${day.next_stop.address || 'Stop'}</p>
                <p class="text-sm mb-1"><strong>Date:</strong> ${day.date || 'N/A'}</p>
                <p class="text-sm"><strong>Distance:</strong> ${day.daily_travel_distance || 0} km</p>
              </div>
            `);
          bounds.extend([stopCoords.lat, stopCoords.lng]);
          goingPoints.push([stopCoords.lat, stopCoords.lng]);
        }
      }
    });
    ...
    if (destCoords && destCoords.lat && destCoords.lng) {
      ...
      const destMarker = L.marker([destCoords.lat, destCoords.lng], { icon: destIcon })
        ...
        .bindPopup(`
          <div class="p-3 min-w-[250px]">
            ...
            <p class="text-sm mb-1"><strong>Coordinates:</strong> ${destCoords.lat?.toFixed(6) || 'N/A'}, ${destCoords.lng?.toFixed(6) || 'N/A'}</p>
            ${destination.weather ? `
      ...
      goingPoints.push([destCoords.lat, destCoords.lng]);
    }

    // Plot return destination if different
    const returnDestCoords = getCoordinates(returnDestination);
    if (returnDestCoords && returnDestCoords.lat && returnDestCoords.lng) {
      ...
      const returnDestMarker = L.marker([returnDestCoords.lat, returnDestCoords.lng], { icon: returnDestIcon })
        .addTo(map)
        .bindPopup(`
          <div class="p-3 min-w-[250px]">
            <div class="flex items-center gap-2 mb-2">
            ...
            <p class="text-sm mb-1"><strong>Coordinates:</strong> ${returnDestCoords.lat?.toFixed(6) || 'N/A'}, ${returnDestCoords.lng?.toFixed(6) || 'N/A'}</p>
            ${returnDestination.weather ? `
              <div class="mt-2 pt-2 border-t border-gray-300">
                <p class="text-sm"><strong>Weather:</strong> ${returnDestination.weather.temperature}°C, ${returnDestination.weather.description}</p>
              </div>
            ` : ''}
          </div>
        `);
      bounds.extend([returnDestCoords.lat, returnDestCoords.lng]);
      returnPoints.push([returnDestCoords.lat, returnDestCoords.lng]);
    }

    // Plot intermediate stops (return) - using daily itineraries for accurate route
    dailyItineraries.forEach((day, dayIndex) => {
      if (day.trip_phase === 'returning' && day.next_stop) {
        const stopCoords = getCoordinates(day.next_stop);
        if (stopCoords && stopCoords.lat && stopCoords.lng) {
          const stopIcon = L.divIcon({
            html: `
              <div class="relative">
                <div class="w-5 h-5 bg-purple-500 rounded-full border-2 border-white shadow-lg flex items-center justify-center">
                  <span class="text-white text-xs font-bold">R${day.phase_day}</span>
                </div>
                <div class="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                  Return ${day.phase_day}
                </div>
              </div>
            `,
            ...
            .bindPopup(`
              <div class="p-3 min-w-[250px]">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <strong class="text-lg">🔄 Return Day ${day.phase_day}</strong>
                </div>
                <p class="text-sm mb-1"><strong>Address:</strong> ${day.next_stop.address || 'Return stop'}</p>
                <p class="text-sm mb-1"><strong>Date:</strong> ${day.date || 'N/A'}</p>
                <p class="text-sm"><strong>Distance:</strong> ${day.daily_travel_distance || 0} km</p>
              </div>
            `);
          bounds.extend([stopCoords.lat, stopCoords.lng]);
          returnPoints.push([stopCoords.lat, stopCoords.lng]);
        }
      }
    });
    ...
    attractions.slice(0, 8).forEach((attraction) => {
      const attractionCoords = getCoordinates(attraction);
      if (attractionCoords && attractionCoords.lat && attractionCoords.lng) {
        ...
        const attractionMarker = L.marker([attractionCoords.lat, attractionCoords.lng], { icon: attractionIcon })
          .addTo(map)
          .bindPopup(`
            <div class="p-2 min-w-[200px]">
              ...
              <p class="text-xs mt-1"><strong>Coordinates:</strong> ${attractionCoords.lat?.toFixed(6) || 'N/A'}, ${attractionCoords.lng?.toFixed(6) || 'N/A'}</p>
            </div>
          `);
        bounds.extend([attractionCoords.lat, attractionCoords.lng]);
      }
    });

    // Plot accommodations (limited to first 5 for clarity)
    accommodations.slice(0, 5).forEach((accommodation) => {
      const accommodationCoords = getCoordinates(accommodation);
      if (accommodationCoords && accommodationCoords.lat && accommodationCoords.lng) {
        ...
        const accommodationMarker = L.marker([accommodationCoords.lat, accommodationCoords.lng], { icon: accommodationIcon })
          .addTo(map)
          .bindPopup(`
            <div class="p-2 min-w-[200px]">
              ...
              <p class="text-xs mt-1"><strong>Coordinates:</strong> ${accommodationCoords.lat?.toFixed(6) || 'N/A'}, ${accommodationCoords.lng?.toFixed(6) || 'N/A'}</p>
            </div>
          `);
        bounds.extend([accommodationCoords.lat, accommodationCoords.lng]);
      }
    });
  ...
  const fitToBounds = () => {
    ...
    if (currentMap && hasTripData) {
      ...
      // Add all points to bounds using the helper function
      const originCoords = getCoordinates(origin);
      const destCoords = getCoordinates(destination);
      const returnDestCoords = getCoordinates(returnDestination);
      ...
      // Add daily itinerary stops
      dailyItineraries.forEach(day => {
        if (day.next_stop) {
          const stopCoords = getCoordinates(day.next_stop);
          if (stopCoords) bounds.extend([stopCoords.lat, stopCoords.lng]);
        }
      });
              ...
              <div className="flex-1 relative">
                ...
                <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm rounded-lg p-4 z-10 border border-gray-200 shadow-lg max-w-xs">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {getCoordinates(returnDestination) && (
                      <div className="flex items-center gap-3">
            ...
            <main className="flex-1 p-6 lg:p-8">
              <div className="flex flex-col h-full max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    {getCoordinates(origin) && (
                      <p className="text-gray-600 text-xs mt-1">
                        {getCoordinates(origin).lat?.toFixed(6) || 'N/A'}, {getCoordinates(origin).lng?.toFixed(6) || 'N/A'}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    ...
                    {getCoordinates(destination) && (
                      <p className="text-gray-600 text-xs mt-1">
                        {getCoordinates(destination).lat?.toFixed(6) || 'N/A'}, {getCoordinates(destination).lng?.toFixed(6) || 'N/A'}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    ...
                    <p className="text-gray-600 text-xs mt-1">
                      {dailyItineraries.length} travel days
                    </p>
                  </div>
                ...
                {dailyItineraries.length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-black font-bold mb-4 text-xl">Trip Itinerary</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {dailyItineraries.map((day, index) => (
                        <div key={index} className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                          <div className="flex justify-between items-start mb-3">
                            <h4 className="text-black font-bold">Day {day.day}</h4>
                            <span className={`text-xs px-2 py-1 rounded ${
                              day.trip_phase === 'going' 
                                ? 'bg-blue-100 text-blue-800' 
                                : 'bg-purple-100 text-purple-800'
                            }`}>
                              {day.trip_phase === 'going' ? 'Going' : 'Return'}
                            </span>
                          </div>
                          <p className="text-gray-600 text-sm mb-2">{day.date}</p>
                          <p className="text-sm mb-2"><strong>Distance:</strong> {day.daily_travel_distance} km</p>
                          <p className="text-sm"><strong>Next Stop:</strong> {day.next_stop?.address || 'Destination'}</p>
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            <p className="text-xs text-gray-500">
                              {day.schedule?.filter(s => s.type === 'attraction').length || 0} activities
                            </p>
                          </div>
                        </div>
                      ))}
...
export default Pr_bd_TravelMaplayout;