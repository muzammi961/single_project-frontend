...
//   const notificationUrl = currentUserId 
//     ? `ws://127.0.0.1:8004/ws/notifications/${currentUserId}/`
//     : null;

//   const { sendMessage: sendNotificationMessage, connected: notificationConnected } = useWebSocket(
//     notificationUrl,
//     (data) => {
...
//           <span className="absolute -bottom-1 -right-1 bg-gray-500 text-white text-xs rounded-full h-3 w-3 flex items-center justify-center">
//             !
//           </span>
//         )}
//       </button>
...
//               <div className="flex items-center gap-2">
//                 <div className={`w-2 h-2 rounded-full ${notificationConnected ? 'bg-green-500' : 'bg-red-500'}`}></div>
//                 {unreadCount > 0 && (
//                   <button
...
//                 {!notificationConnected && (
//                   <p className="text-xs text-amber-400 mt-1">Reconnecting...</p>
//                 )}
//               </div>
//             )}
//           </div>
...
//   // Experience Detail Modal Component with WebSocket Integration
...
//     // WebSocket URLs
//     const commentWsUrl = postId ? `ws://127.0.0.1:8004/ws/comments/${postId}/` : null;
//     const likeWsUrl = postId ? `ws://127.0.0.1:8004/ws/likes/${postId}/` : null;

//     // WebSocket for real-time comments
//     const { sendMessage: sendCommentMessage, connected: commentConnected } = useWebSocket(
//       commentWsUrl,
//       (data) => {
//         console.log('💬 New comment via WebSocket:', data);
...
//     const { sendMessage: sendLikeMessage, connected: likeConnected } = useWebSocket(
//       likeWsUrl,
//       (data) => {
//         console.log('❤️ Like update via WebSocket:', data);
...
//     // Handle adding comment via WebSocket
...
//         const success = sendCommentMessage(commentData);
...
//     // Fallback: Add comment via REST API
//     const addCommentViaRestAPI = async () => {
//       try {
...
//         if (response.data) {
//           console.log('✅ Comment added successfully via REST API:', response.data);
//           // Refresh comments to get the latest
//           await fetchComments();
//           setNewComment("");
//         }
//       } catch (error) {
//         console.error("❌ Error adding comment via REST API:", error);
//         console.error("Error details:", error.response?.data);
//         alert(`Failed to post comment: ${error.response?.data?.message || error.message}`);
//       }
//     };

//     // Handle toggling like via WebSocket
...
//       if (!likeConnected) {
//         console.warn('WebSocket not connected, using REST API fallback');
//         await toggleLikeViaRestAPI();
//         return;
//       }

//       setLikeLoading(true);
//       try {
//         console.log('📤 Sending like via WebSocket...');
//         const likeData = {
//           user_id: parseInt(currentUserId)
//         };
        
//         const success = sendLikeMessage(likeData);
...
//     // Fallback: Toggle like via REST API
//     const toggleLikeViaRestAPI = async () => {
//       try {
...
//         if (response.data) {
//           console.log('✅ Like toggled successfully via REST API:', response.data);
//           // Update like status
//           setHasLiked(response.data.liked);
//           setLikeCount(response.data.like_count);
//         }
//       } catch (error) {
//         console.error("❌ Error toggling like via REST API:", error);
//         console.error("Error details:", error.response?.data);
//       }
//     };
...
//                   <p>No comments yet. Be the first to comment!</p>
//                   {!commentConnected && (
//                     <p className="text-xs text-amber-400 mt-1">Reconnecting WebSocket...</p>
//                   )}
//                 </div>
//               )}
//             </div>
...
//                 </p>
//               )}
//               {token && !commentConnected && (
//                 <p className="text-xs text-amber-500 mt-2 text-center">
//                   Using fallback mode - comments may be delayed
//                 </p>
//               )}
//             </div>
//           </div>
...
//       navigate("/LoginPage");
//     }
//   }, [token, fetchCounts, ProfiledataFunc, navigate]);
