...
const UpcomingTripsSection = () => {
  const [upcomingTrips] = useState([
    {
      id: 1,
      travelName: "European Adventure",
      destination: "London, UK",
      return_destination: "Paris, France",
      image:
    ...
    {
      id: 2,
      travelName: "Island Escape",
      destination: "Bali, Indonesia",
      return_destination: "Jakarta, Indonesia",
      image:
  ...
  const TripCard = ({ trip }) => {
    return (
      <div className="relative rounded-2xl overflow-hidden shadow-xl border border-pink-500 bg-gradient-to-b from-purple-900 via-orange-800 to-yellow-600 p-[2px] hover:scale-[1.02] transition-all duration-500">
        {/* Inner content */}
        <div className="bg-gradient-to-b from-orange-900/70 via-gray-900/80 to-black/90 rounded-2xl text-center text-white py-10 px-6 flex flex-col items-center justify-center min-h-[340px]">
          <div className="relative w-full h-40 rounded-lg overflow-hidden mb-5">
            <img
              src={trip.image}
              alt={trip.destination}
              className="absolute inset-0 w-full h-full object-cover opacity-80 hover:opacity-100 transition-all duration-500"
            />
          </div>
          ...
          <p className="text-lg text-orange-200 mb-4 font-medium">
            {trip.destination} → {trip.return_destination}
          </p>

          <p className="text-sm text-yellow-300 mb-2">
            📅 {trip.date} | 🕓 {trip.duration}
          </p>
          <p className="text-sm text-green-300 mb-4 font-semibold">
            💰 Budget: {trip.budget}
          </p>

          <div className="mt-auto text-xs text-gray-300 italic">
            “Savor every journey’s beauty”
          </div>
        </div>
      </div>
    );
