...
const ExperienceSide = () => {
  ...
  const [commentData, setCommentData] = useState({ content: '' });
  const [selectedExperience, setSelectedExperience] = useState(null);
  const [placeExperiences, setPlaceExperiences] = useState([]);
  const [selectedPlaceForExperiences, setSelectedPlaceForExperiences] = useState(null);
    ...
    const fetchExperiences = async () => {
      try {
        ...
        const adaptedExperiences = apiExperiences.map(exp => ({
          ...
          title: exp.title,
          placeId: exp.place_name, // Use as string for search
  ...
  const fetchPlaceExperiences = async (placeName) => {
    try {
      const res = await axios.post("http://127.0.0.1:8004/TravelExperienceSearchAPIView/", { place_name: placeName }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const apiExperiences = res.data.results || [];
      // Adapt API data to experience structure
      ...
      setPlaceExperiences(adaptedExperiences);
    } catch (error) {
      console.error("Error fetching place experiences:", error);
    }
  };
  ...
  const filteredExperiences = searchType === 'Experiences' && searchQuery
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col justify-between bg-black font-display text-white">
            ...
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
              <div className="bg-gray-800/50 backdrop-blur-lg rounded-xl p-8 w-full max-w-xl shadow-lg shadow-teal-500/20 max-h-[80vh] overflow-y-auto scrollbar-thin scrollbar-thumb-teal-500 scrollbar-track-gray-700">
                ...
                {selectedExperience.image ? (
                  ...
                  <div className="w-full h-64 bg-gray-700 flex items-center justify-center rounded-lg mb-4 text-gray-400">
                    No Image
                  </div>
                )}
                <p className="text-base text-gray-200 mb-3">{selectedExperience.content}</p>
      ...
      <footer className="sticky bottom-0 bg-black/80 backdrop-blur-lg border-t border-teal-500/50 shadow-lg shadow-teal-500/20">
        <nav className="flex justify-around items-center p-4">
          <a onClick={() => navigate('/ExperienceSide')} className="flex flex-col items-center gap-1 text-teal-400 hover:text-teal-300 transition-colors duration-200" href="#">
            <span className="material-symbols-outlined text-2xl">explore</span>
            <span className="text-sm font-medium">Experience</span>
          </a>
          <a onClick={() => navigate('/TripPlanner')} className="flex flex-col items-center gap-1 text-gray-200 hover:text-gray-100 transition-colors duration-200" href="#">
            <span className="material-symbols-outlined text-2xl">receipt_long</span>
            <span className="text-sm font-medium">Expense</span>
          </a>
          <a className="flex flex-col items-center gap-1 text-gray-200 hover:text-gray-100 transition-colors duration-200" href="#">
